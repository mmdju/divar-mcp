...
- **هر عددی که تو فیلد قیمته، قیمت نیست.** فروشنده‌ای که قیمت نمی‌ده یه عدد الکی می‌زنه (`۱,۰۰۰ تومان`، ارقام تکراری) - اینا پنهان نمی‌شن ولی **`price_is_placeholder`** می‌گیرن با یه `price_note` ساده، و هیچ‌وقت میانه رو تعیین نمی‌کنن. شاهدش هم به شکل **`price_reading`** میاد تا خودت درباره‌ی عدد قضاوت کنی.
- **یه آگهی اجاره دو تا عدد داره.** `price_toman` اجاره‌ست و `deposit_toman` (ودیعه) بغلش میاد، هر کدوم با پرچم خودش. اتاق همخونه (`همخونه` / `هماتاقی` / `اجاره اتاق`) هم **`shared_housing`** می‌گیره - قیمت یه اتاق، اجارهٔ یه واحد نیست.
...
- **`market_price` کارنامه نیست.** خودش می‌گه چند آگهی رو سنجیده و قیمت‌های الکی رو از حساب بیرون می‌ذاره.
