...
- **Every cut is published as its own reading of the same pool.** The cuts stay - a median that mixes rooms with flats is wrong, and a 1-Toman listing is not a price - but they are no longer the only reading a caller can get: **`sample_views`** holds the same pool under each one (`as_fetched`, `honest_asks`, `whole_units` on a rent search, and `honest_whole_units` when both apply), each with `left_out_ads`, while `sample_view_used` names which reading the `sample` numbers above are and `sample_views_left_out` names up to five tokens each cut dropped ("excluded" means out of this arithmetic, never out of the answer). The choice of reading belongs to the caller - the fields just make it a choice.
