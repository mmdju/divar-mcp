...
- **A rent median is the median of whole units.** A room share (`همخونه` / `هماتاقی`, read from the ad's **own title**) is a real listing, but its number is a room's price rather than a flat's rent, so when the sample still holds five priced whole units those ads are kept out of the median, the quartiles and the cheapest ads - `shared_housing_excluded` is how many - and every one of them is still shown in **`sample_ads_shared_housing`** with its own note. Measured live 2026-09-21: 5 of 30 ads in a low-deposit Tehran rent search were room shares, and leaving them in moved the median rent from 14,000,000 to 16,500,000 Toman (17.9%) - the difference between "around the market" and "over it" for an honest flat. With fewer than five whole units left they stay in the maths and `shared_housing_note` says the median reads softer.
